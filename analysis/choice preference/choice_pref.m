...
ren_count_i = 0;
    ...
    for index=1:length(subjs)
        ...
        acq_trials_2 = current_subj.pick_trials(41:60);
        ext_trials_2 = current_subj.pick_trials(61:75);
        ren_trials = current_subj.pick_trials(76:80);
        ...
        acq_2 = [acq_2 length(corr_acq_2.trials)/20];
        ext_2 = [ext_2 length(corr_ext_2.trials)/15];
        ren = [ren length(corr_ren.trials)/5];
        ...
        acq_inc = [acq_inc length(incorr_acq_2.trials)/20];
        ext_inc = [ext_inc length(incorr_ext_2.trials)/15];
        ren_inc = [ren_inc length(incorr_ren.trials)/5];
    ...
    ylabel('Proportion');
    title('Across Phases (behavioral)');
    legend('Preference for learned choice', 'Preference for alternative choice');
    
    txt = '***';
    text(1.6,0.72,txt,'FontSize',15, 'HorizontalAlignment','center', 'Color', [0 0.4470 0.7410]);
    a=4;
end
